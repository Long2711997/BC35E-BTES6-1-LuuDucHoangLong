const colorList = ["pallet", "viridian", "pewter", "cerulean", "vermillion", "lavender", "celadon", "saffron", "fuschia", "cinnabar", "purple"];
...
let renderButtonString = () => {
    ...
    for (let index = 0; index < colorList.length; index++){
        ...
        content += `
        <button style="background-color: ${color};" class="btn ${color}" onclick="changeColor('${color}')">${color}</button>`
    }
    document.getElementById('colorContainer').innerHTML = content;
}

window.changeColor = (color) => {
    let change = `<div id="house" class="house ${color}">
    ...
    document.getElementById('house').innerHTML = change;
}
